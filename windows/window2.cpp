...
static unsigned char pvkey_codes[256][8][16];
...
static void init_fonts(int pick_width, int pick_height)
{
  ...
  //   SetTextColor(und_dc, D2D_RGB(255, 255, 255));
  //   SetBkColor(und_dc, D2D_RGB(0, 0, 0));
  ...
  for (key = 0; key < 256; key++) {
    ...
    for (mod = 0; mod < 8; mod++) {
      for (len = 0; (t < max) && (buf[t] != ',') && (len < 15); len++, t++) {
        if (buf[t] == '\\') {
          sscanf(&buf[++t], "%hho", &pvkey_codes[key][mod][len]);
          t += 2;
          continue;
        }
                      ...
                      unsigned long long attr, int lattr, truecolour truecolour)
{
  ...
  if (!pal && truecolour.fg.enabled)
    fg = D2D_RGB(truecolour.fg.r, truecolour.fg.g, truecolour.fg.b);
  else
    fg = colours[nfg];

  if (!pal && truecolour.bg.enabled)
    bg = D2D_RGB(truecolour.bg.r, truecolour.bg.g, truecolour.bg.b);
  else
    bg = colours[nbg];

  if (!pal && (attr & ATTR_DIM)) {
    fg = D2D_RGB(GetRValue(fg) * 2 / 3,
             GetGValue(fg) * 2 / 3,
             GetBValue(fg) * 2 / 3);
  }
...
#define SHOW_TOASCII_RESULT
...

    {
      ...
      if (length) {
        strcpy((char *) output, (char *)pvkey_codes[wParam][mod]);
        return length;
      }
    }
      ...
      for (i = 0; i < (len - 1); i++) {
        ...
        for (i = 0; i < (len-1); i++) {
          if (truecolour[i].fg.enabled) {
            ...
            rgbp->ref = D2D_RGB(truecolour[i].fg.r,
                            truecolour[i].fg.g,
                            truecolour[i].fg.b);
          ...
          if (truecolour[i].bg.enabled) {
            rgbindex *rgbp = snew(rgbindex);
            rgbp->ref = D2D_RGB(truecolour[i].bg.r,
                            truecolour[i].bg.g,
                            truecolour[i].bg.b);
            if (add234(rgbtree, rgbp) != rgbp)
              sfree(rgbp);
    ...
    while (tindex < len2 && uindex < len && tdata[tindex] && udata[uindex]) {
      ...
      if (attr) {
        ...
        if (truecolour && truecolour[tindex].fg.enabled) {
          fgcolour = -1;
          fg = D2D_RGB(truecolour[tindex].fg.r,
        ...
        if (truecolour && truecolour[tindex].bg.enabled) {
          bgcolour = -1;
          bg = D2D_RGB(truecolour[tindex].bg.r,
...
void set_colour(void *frontend, int number, int colour)
{
  ...
  colours[number] = D2D_RGB((colour >> 16) & 0xff,
                        (colour >> 8) & 0xff,
                        (colour >> 0) & 0xff);
  InvalidateRect(hwnd, NULL, TRUE);
  return;
...
static void bg_glass()
{
  ...
  if (osMajorVersion > 9) {
    ...
    if (!p_SetWindowCompositionAttribute) {
      ...
      if (hmLib) {
        GET_WINDOWS_FUNCTION_NO_TYPECHECK(hmLib, SetWindowCompositionAttribute);
      }
    }
